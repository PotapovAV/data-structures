...
func TestOSTree(t *testing.T) {
	...
	tree.Insert(createTestData(7, "seven"))

	sorted := tree.InorderTreeWalk()
	assert.Equal(t, 7, len(sorted), "7 should be the length of sorted slice")

	node := tree.Search(8)
	assert.Equal(t, (*Node)(nil), node, "nil should be returned by tree.Search(8)")
	...
	node = tree.Maximum()
	assert.Equal(t, 7, node.Data().(testData).key, "7 should be returned by tree.Maximum()")
	...
	node = tree.Select(6)
	assert.Equal(t, (*Node)(nil), node, "nil should be returned by tree.Select(6)")

	node = tree.Select(0)
	assert.Equal(t, 1, node.Data().(testData).key, "1 should be returned by tree.Select(0)")

	node = tree.Select(5)
	assert.Equal(t, 7, node.Data().(testData).key, "7 should be returned by tree.Select(5)")
